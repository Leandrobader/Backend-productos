    ...
    async Login(req,res){
        try {
            ...
            if(body.password === "" || body.password === undefined){
                throw new Error("debe enviar password")
            }

            const user = await UserModel.findOne({email:body.email}); //buscamos el usuario

            if(user === null){
                return res.status(404).json({message: "Email y/o password incorrectos"});
            }

            const compare= await bcrypt.compare(body.password, user.password) //comparamos el password que viene del front con el de la base de datos que esta almacenado en user
